...
async def _async_put_signalling(
    q: asyncio.Queue,
    item,
    done_event: "threading.Event",
) -> None:
    """
    Put item into the asyncio queue, then signal the MTProto thread that
    space was consumed (or the put itself completed).
    This is the backpressure mechanism: the MTProto _download_task waits
    on done_event before fetching the next chunk from Telegram, so it never
    floods the queue with more data than the client can consume.
    """
    await q.put(item)
    done_event.set()
...
async def iter_large_file(
    ...
    def put_from_thread(item) -> None:
        """
        Thread-safe put with backpressure.

        BUFFERING ROOT CAUSE (old code):
          run_coroutine_threadsafe was fire-and-forget — _download_task
          immediately fetched the next 8 MB chunk from Telegram without
          waiting for the queue to have space. This caused a burst of up to
          8 × 8 MB = 64 MB to be fetched and queued at once, followed by a
          long stall while VLC consumed the burst. VLC showed this as
          intermittent buffering.

        FIX:
          We use a threading.Event as a one-shot signal. After scheduling
          _async_put_signalling on the uvicorn loop, the MTProto thread
          (this function's caller) BLOCKS on done_event.wait() until the
          item has actually been placed in the queue. This means Telethon
          only fetches the next chunk after the previous one is in the
          queue — natural backpressure that keeps the buffer small and
          the data flow smooth and continuous.
        """
        done_event = threading.Event()
        asyncio.run_coroutine_threadsafe(
            _async_put_signalling(data_queue, item, done_event),
            uvicorn_loop,
        )
        # Block the MTProto thread until the item is in the queue.
        # Timeout of 60s prevents infinite hang if uvicorn dies mid-stream.
