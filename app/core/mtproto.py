...
async def _download_task(
    ...
    try:
        ...
        logger.info(
            "MTProto download start: chat=%s msg=%s offset=%d length=%s request_size=%s KB",
            chat_id, message_id, offset, length, TELETHON_REQUEST_SIZE // 1024,
        )

        async for chunk in client.iter_download(
            msg.media,
            offset=offset,
            chunk_size=TELETHON_REQUEST_SIZE,
            request_size=TELETHON_REQUEST_SIZE,
        ):
...
async def iter_large_file(
    ...
    sentinel     = object()
    data_queue: asyncio.Queue   = asyncio.Queue(maxsize=MTPROTO_QUEUE_CHUNKS)
