...
# VLC and browsers send multiple Range requests per file (to parse container
# headers, seek tables, etc.). Each request calls iter_large_file() which
# calls get_messages() — a round-trip to Telegram DC (~300-500 ms each).
# With 3-5 Range requests for an MKV file, that is 1-2.5 s of pure latency
# BEFORE any video data flows, which VLC shows as "buffering".
#
# Fix: cache the message object by (chat_id, message_id).
# First request fetches from Telegram and stores it.
# All subsequent Range requests for the same file get it from cache (0 ms).
# Cache holds up to 64 entries with a 6-hour TTL matching the link expiry.
import time as _time
from threading import Lock as _Lock

_msg_cache: dict = {}          # (chat_id, message_id) → (msg, fetched_at)
_msg_cache_lock   = _Lock()
_MSG_CACHE_TTL    = 21600      # 6 hours — matches DEFAULT_TTL_SECONDS
_MSG_CACHE_MAX    = 64         # max entries before evicting oldest


def _cache_get(chat_id: int, message_id: int):
    key = (chat_id, message_id)
    with _msg_cache_lock:
        entry = _msg_cache.get(key)
        if entry and (_time.time() - entry[1]) < _MSG_CACHE_TTL:
            return entry[0]
        if entry:
            _msg_cache.pop(key, None)   # expired
    return None


def _cache_set(chat_id: int, message_id: int, msg) -> None:
    key = (chat_id, message_id)
    with _msg_cache_lock:
        # Evict oldest entry if at capacity
        if len(_msg_cache) >= _MSG_CACHE_MAX and key not in _msg_cache:
            oldest = min(_msg_cache, key=lambda k: _msg_cache[k][1])
            _msg_cache.pop(oldest, None)
        _msg_cache[key] = (msg, _time.time())
...
async def _download_task(
    ...
    try:
        ...
        # Use cached message if available — avoids a 300-500 ms Telegram
        # round-trip on every Range request (VLC sends 3-5 per file).
        msg = _cache_get(chat_id, message_id)
        if msg is None:
            msg = await client.get_messages(chat_id, ids=message_id)
            if not msg or not msg.media:
                raise LargeFileStreamError("Could not find media message")
            _cache_set(chat_id, message_id, msg)
            logger.info("Message fetched and cached: chat=%s msg=%s", chat_id, message_id)
        else:
            logger.debug("Message served from cache: chat=%s msg=%s", chat_id, message_id)
        if not msg or not msg.media:
            raise LargeFileStreamError("Could not find media message")
