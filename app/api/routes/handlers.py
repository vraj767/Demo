...
CHANGES:
  5. All error conditions (404, 410, 429, 400, 503) now return styled dark-theme
     HTML pages instead of bare JSON. Each page shows the error code, a friendly
     message, and a "← Go Back" button. CSS variables match the file page.
"""
import logging
...
def _error_page(code: int, title: str, message: str) -> HTMLResponse:
    """
    Returns a styled dark-theme HTML error page.
    Reuses the same CSS variables as the file page:
      --bg, --card, --border, --text, --muted, --accent, --orange
    """
    icon_map = {
        400: "⚠️",
        404: "🔍",
    ...
    try:
        ...
        if "not ready" in msg.lower():
            return _error_page(503, "Server Not Ready",
                "The streaming server is still warming up. Please wait a few seconds and try again.")
