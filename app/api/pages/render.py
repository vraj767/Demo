...
def render_file_page(file_hash: str, info: dict) -> str:
    ...
    # 1DM intent URL — carefully constructed to open 1DM directly, not Chrome.
    #
    # KEY: Do NOT include ;action=android.intent.action.VIEW
    # With VIEW action, Android asks "which app handles HTTPS URLs?" and Chrome
    # wins because it's the default browser. Without action, Android routes
    # directly to the named package (1DM) with no ambiguity or chooser dialog.
    #
    # Parameters:
    #   scheme=https        — tells 1DM to use HTTPS to fetch the URL
    #   package=...adm      — 1DM's real package name (not idm.internet.download.manager)
    #   type=...            — MIME hint so 1DM treats it as a binary download
    #   S.filename=...      — pre-fill 1DM's filename field
    #   S.browser_fallback_url — if 1DM not installed, open direct download in browser
    _dl_no_scheme = download_url.replace("https://", "").replace("http://", "")
    _fname_enc    = info.get("file_name", "download").replace(" ", "%20")
    dm1_deep = (
...
<script>
    ...
    // if/else must be together — no function declarations between them
    if(isAndroid){{
        document.getElementById('opt-1dm').style.display='flex';
    }}else{{
        document.getElementById('tip-speed-text').innerHTML='<strong>Pro Tip:</strong> Use <strong>VLC</strong> for best playback, or copy the Source URL into any download manager.';
    }}
